...
//   GET ?pull=<id>              → the full snapshot JSON for one pull
//
...
// totals; the same figures a logged-in investor already sees).
import { listSnapshotsWithData, readSnapshot } from "../lib/store.js";
...
export default async function handler(req, res) {
  ...
  if (req.query?.pull) {
    const snap = await readSnapshot(req.query.pull);
