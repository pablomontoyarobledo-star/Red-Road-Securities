...
export async function getAccountActivity(sql, { accountCode, from, to }) {
  return await sql`
    select je.entry_date as date, je.memo, je.source_type, je.source_id,
           jl.debit, jl.credit, jl.investor_id, jl.description
    from journal_lines jl
    join journal_entries je on je.id = jl.entry_id
...
export async function listExpenses(sql, { from = null, to = null } = {}) {
  if (from && to) {
    return await sql`
      select entry_date as date, category, amount_usd as amount, description, status, id
