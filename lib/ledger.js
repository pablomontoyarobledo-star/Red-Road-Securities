...
// Flat journal listing for the General Ledger UI — one row per journal_line,
// joined to its parent entry and account name, newest-first, capped at
// `limit` (clamped server-side regardless of what the caller sends, since
// this is the one read path here driven by open-ended client input rather
// than a fixed date/account). Same casting discipline as every other read
// helper in this file. `from`/`to` are both optional — unlike
// getAccountActivity (which requires an exact account+range), the General
// Ledger should work with no filter at all (defaults to the most recent
// `limit` rows).
export async function listJournalEntries(sql, { from = null, to = null, limit = 300 } = {}) {
  const cappedLimit = Math.min(Math.max(Number(limit) || 300, 1), 500);
  return await sql`
    select je.id as entry_id, je.entry_date::text as date, je.memo,
           je.source_type, je.source_id, je.reverses_entry_id,
           jl.id as line_id, jl.account_code, a.name as account_name,
           jl.investor_id, jl.debit::float8 as debit, jl.credit::float8 as credit,
           jl.description
    from journal_lines jl
    join journal_entries je on je.id = jl.entry_id
    join gl_accounts a on a.code = jl.account_code
    where (${from}::date is null or je.entry_date >= ${from})
      and (${to}::date is null or je.entry_date <= ${to})
