...
// ── Unit transfers (management fee / bonus paid in kind) ───────────────────
//
// A unit transfer moves ownership between two investors — e.g. Fernando's
// monthly management fee, settled by handing Pablo units instead of cash —
// without changing total fund NAV or total units outstanding. It is
// deliberately NOT folded into depositCash/computeInvestorUnitsAtDate/
// buildUnitSchedule above: those assume every record is a cash inflow tied
// to a settlement date, which a transfer isn't, and recomputeNavSeries'
// pricing (NAV = totalValue / totalUnits) must stay ignorant of transfers —
// they're zero-sum fund-wide by construction (both legs use the same
// amount/nav), so total units outstanding is already correct without this
// file touching it at all. Only per-investor attribution (ownership %,
// statements) needs to know about them — that's what these functions add.
//
// Shape of one unit-transfer record: { date, type, from, to, amountUsd,
// navPerUnit, units, description }, where `from`/`to` are investor keys in
// the same convention investorKeysFor()/depositBelongsTo() already use.

// True if a unit-transfer record involves an investor identified by `keys`
// (from investorKeysFor) — the transfer analog of depositBelongsTo.
export function unitTransferBelongsTo(t, keys) {
  return keys.has(t.from) || keys.has(t.to);
}

// Net units an investor has gained/lost via unit transfers as of a date —
// positive if they've received more than they've paid (e.g. Pablo), negative
// otherwise (e.g. Fernando, Dario).
export function computeInvestorUnitTransferNet(unitTransfers, investor, dateStr) {
  const keys = investorKeysFor(investor);
  return (unitTransfers || [])
    .filter(t => !dateStr || t.date <= dateStr)
    .reduce((sum, t) => {
      if (keys.has(t.to))   sum += t.units;
      if (keys.has(t.from)) sum -= t.units;
      return sum;
    }, 0);
}

// Investor's units as of a date, including both deposits and unit transfers
// — the transfer-aware analog of computeInvestorUnitsAtDate. Use this (not
// the plain version) anywhere a management fee/bonus needs to already be
// reflected in an investor's ownership — per-investor statements, ownership
// %, and the monthly fee-split calculation itself (so July's split correctly
// nets in June's transfer instead of overstating the payer's share).
export function computeInvestorUnitsAtDateWithTransfers(deposits, unitTransfers, investor, dateStr) {
  return computeInvestorUnitsAtDate(deposits, investor, dateStr)
       + computeInvestorUnitTransferNet(unitTransfers, investor, dateStr);
}
