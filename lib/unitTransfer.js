...
//
// Explicit casts matter here: the neon driver returns Postgres `numeric`
// columns as JS strings (not numbers) and `date` columns as JS Date objects
// (not "YYYY-MM-DD" strings) unless told otherwise — either would silently
// break every consumer of this data (string concatenation instead of
// arithmetic on the numerics; Date's quirky `<=`/`.toString()` behavior
// breaking every string date comparison in lib/nav.js, since Date is the one
// JS type where the "default" ToPrimitive hint behaves like "string" rather
// than "number"). Cast to the exact shape every other part of this codebase
// already assumes (plain numbers, plain "YYYY-MM-DD" strings).
export async function listUnitTransfers(sql) {
  return await sql`
    select effective_date::text as date, type, from_investor_id as "from", to_investor_id as "to",
           amount_usd::float8 as "amountUsd", nav_per_unit::float8 as "navPerUnit",
           units::float8 as units, description
...
export async function checkAndPostMonthlyFees(sql, { actor }) {
  ...
  for (const month of months) {
    ...
    const prior = await sql`
      select effective_date::text as date, type, from_investor_id as "from", to_investor_id as "to", units::float8 as units
