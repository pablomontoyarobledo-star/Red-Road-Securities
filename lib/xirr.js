...
export function xirr(cashflows) {
  ...
  // A total wipeout (no positive cash flow at all, e.g. current value is
  // exactly $0) has no finite rate that zeroes the NPV — dividing zero by any
  // discount factor is still zero, so NPV stays at the (negative) sum of
  // outflows for every r. The correct answer is the limit, -100%.
  if (!cashflows.some(cf => cf.amount > 0)) return -1;
