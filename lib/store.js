...
export async function ensureSchema() {
  ...
  //
  // The accounting system of record. gl_accounts is seeded once by
  // scripts/seed-chart-of-accounts.mjs; journal_entries/journal_lines are
  // append-only (see lib/ledger.js — never UPDATEd or DELETEd, only
  // reversed via a new correcting entry). unit_transfers and expense_entries
  // are the two things that actually create journal entries: an IB-sourced
  // or manual expense, and the monthly management-fee/bonus unit transfer
  // (see lib/unitTransfer.js).
  await sql`
  ...
  `;
  // The idempotency gate that makes re-running the IB cron (or re-importing
  // the same XML) safe: the same IB event or manual action can never post a
  // second journal entry, no matter how many times the caller retries.
  await sql`
  ...
  `;
  await sql`create index if not exists journal_entries_date_idx on journal_entries (entry_date)`;
  await sql`
  ...
  await sql`create index if not exists journal_lines_account_idx on journal_lines (account_code, entry_id)`;
  await sql`create index if not exists journal_lines_investor_idx on journal_lines (investor_id) where investor_id is not null`;

  // ── Unit transfers (management fee / bonus paid in kind) ───────────────
  //
  // One row per payer→payee leg (mirrors the deposits idiom: one row per
  // investor per event). nav_per_unit/units are stored, not recomputed on
  // read, so a later NAV restatement never silently reprices a past fee.
  // source_key gives per-investor-per-month idempotency:
  // "${type}|${effectiveDate}|${fromInvestorId}".
  await sql`
    create table if not exists unit_transfers (
      id               bigint generated always as identity primary key,
      effective_date   date not null,
      type             text not null check (type in ('management_fee','bonus','other')),
  ...
  `;
  await sql`create unique index if not exists unit_transfers_source_key_uidx on unit_transfers (source_key)`;
  await sql`create index if not exists unit_transfers_date_idx on unit_transfers (effective_date)`;
  await sql`create index if not exists unit_transfers_investor_idx on unit_transfers (from_investor_id, to_investor_id)`;

  // ── Manual expenses (non-IB: custodial, audit, legal, other) ───────────
  await sql`
    create table if not exists expense_entries (
      ...
      amount_usd       numeric(18,2) not null check (amount_usd > 0),
      description      text,
      status           text not null default 'paid' check (status in ('paid','accrued')),
      journal_entry_id bigint not null references journal_entries(id),
      created_by       text not null,
      created_at       timestamptz not null default now()
    )
  `;
  await sql`create index if not exists expense_entries_date_idx on expense_entries (entry_date)`;
}
